...
func (w *InventoryRestoreWorker) process(ctx context.Context) {
	...
	jobs, err := w.inventoryJobGormRepo.FindDueJob(pollCtx, 300)
	pollCancel()

	if err != nil {
		w.sendNotification(ctx, fmt.Sprintf("failed to find due job: %s", err.Error()))
		return
	}
	...
	for _, job := range jobs {
		...
		case productdomain.TargetDB:
			w.handleDBJob(jobCtx, job)
		case productdomain.TargetRedis:
			w.handleRedisJob(jobCtx, job)
		default:
			if err = w.updateInventoryJob(jobCtx, job, "unknown restore target", productdomain.JobFailed); err != nil {
...
func (w *InventoryRestoreWorker) handleDBJob(ctx context.Context, job productdomain.InventoryJob) {
	...
	default:
		...
		if err := w.updateInventoryJob(ctx, job, "unknown operation", productdomain.JobFailed); err != nil {
			w.sendNotification(ctx, fmt.Sprintf("failed to update inventory job: %v", err))
		}
	}
	return
}
...
func (w *InventoryRestoreWorker) sendNotification(ctx context.Context, message string) {
	...
	slog.ErrorContext(notifyCtx, "inventory worker notification", "message", message)
	_ = w.slackSender.Send(notifyCtx, notification.Message{
